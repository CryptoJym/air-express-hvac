...
| Live GA4 is split by host | live_verified | `npm run verify:live-measurement` writes `data/live-measurement-path-check.json`; latest status is `partial_live_www_only_canonical_pending`. `https://www.airexpressutah.com/` now exposes `G-JZ7PY32EVX` and `https://www.airexpressutah.com/analytics.js` returns HTTP 200. Canonical `https://airexpressutah.com/` still serves the older New Reward edge/origin path on website id `cmorqbs9j001r5nr1h25vosp8`; canonical `/analytics.js` returns HTTP 404 and the apex homepage does not expose the approved GA4 marker. |
| Cloudflare Turnstile source is prepared but not active | repo_verified / live_verified | Git commit `27cf80099165065e7f5bd0c0ca49d595326c4c44` added `api/_lib/turnstile.js`, `api/turnstile/config.js`, intake Turnstile rendering, and server-side token verification on 2026-06-04. Live checks on 2026-06-05 show `https://www.airexpressutah.com/api/turnstile/config` returns `configured:false` with missing `TURNSTILE_SITE_KEY` and `TURNSTILE_SECRET_KEY`; canonical `https://airexpressutah.com/api/turnstile/config` returns HTTP 404 on the New Reward edge path. Treat CAPTCHA as source-ready but not active, and do not claim lead-quality impact yet. |
| Owned-site delivery sync is pending on canonical apex | live_verified | `npm run verify:owned-site-delivery-sync` writes `data/owned-site-delivery-sync-check.json`; latest status is `pending_delivery_sync`. Local `index.html` and `analytics.js` contain the approved GA4 source, and the Vercel/www path now serves it, while canonical apex homepage and `/analytics.js` are still missing the critical measurement artifact. Live `llms.txt`, `sitemap.xml`, and `robots.txt` currently differ from repo-local files or are edge-managed. |
...
| ServiceTitan lead history is pulled through the option-c credential path | repo_verified | `npm run verify:servicetitan-export` now writes `data/servicetitan-export-access-check.json` with status `auth_ready_no_export` after pulling the separate Vercel `option-c` production env into a temp-only file. `npm run pull:servicetitan-lead-history` writes `data/servicetitan-api-lead-history-summary.json` and `data/servicetitan-api-lead-history-redacted.csv` with 78 redacted lead rows from 2026-01-01 through 2026-06-05, including 76 configured website campaign rows, service-type buckets, weekly zero-fill through the week beginning 2026-06-01, and CAPTCHA source-period markers. Stored fields exclude customer names, phone numbers, emails, addresses, summaries, lead URLs, raw payloads, credential values, and tokens. Booking and revenue joins are intentionally out of scope for the current Air Express report. |
| ServiceTitan booking/revenue join probe is historical background, not a current blocker | repo_verified / official_doc | `npm run probe:servicetitan-attribution-joins -- --from 2020-01-01` previously wrote `data/servicetitan-attribution-join-probe.json`. It confirms CRM export bookings is readable but returned 0 rows from the widened cursor, while JPM export jobs and Accounting export invoices, invoice items, and payments returned HTTP 403 `Scope validation failed` blockers. This evidence remains useful if James reopens booked-job/revenue attribution later, but it is not required for the current lead-count report. |
