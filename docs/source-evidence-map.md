...
| Google provider account is local but under-scoped | repo_verified | `npm run pull:gsc-ga4-history` confirms `newrewardplatform@gmail.com` is available through local token sources. The sanitized scope inventory shows cloud/admin identity scopes, but neither token has `webmasters.readonly`, `analytics.readonly`, or `business.manage`. |
...
| ServiceTitan export access is blocked locally | repo_verified | `npm run verify:servicetitan-export` writes `data/servicetitan-export-access-check.json` with status `blocked_env`; required ServiceTitan env keys are absent across the Air Express repo, global New Reward config, home env, and the checked NewRewards local/Vercel env files. The diagnostic stores only key-presence/source labels and no credential values, tokens, customer PII, or raw lead payloads. Sanitized Gmail evidence preserves April 24 lead IDs, an April 29 ServiceTitan email-notification fallback setup note, and a May 1 lead-visibility investigation record. A June 3 mailbox search found no full ServiceTitan export, booking status, revenue report, or new lead IDs beyond existing proof. These are historical setup/visibility facts, not a full lead export, booking report, or revenue report. |
