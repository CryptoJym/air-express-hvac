...
| Live GA4 source | `www.airexpressutah.com` now serves `G-JZ7PY32EVX` and `/analytics.js` HTTP 200, but canonical `airexpressutah.com` still serves the older New Reward edge/origin path with no GA4 marker and canonical `/analytics.js` HTTP 404. `data/owned-site-delivery-sync-check.json` still records canonical homepage and `analytics.js` as `pending_delivery_sync`. | Canonical edge/routing sync and post-sync verification. |
| Turnstile / CAPTCHA | Source code is present, but `www.airexpressutah.com/api/turnstile/config` reports missing `TURNSTILE_SITE_KEY` and `TURNSTILE_SECRET_KEY`; canonical apex returns HTTP 404 for the config endpoint. | Approved Turnstile site/secret keys in the active production delivery environment, then post-sync verification. |
